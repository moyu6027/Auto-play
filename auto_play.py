...
class AutoTask:
    ...
    def create_sshclient(self):
        ...
        try:
            ...
            self.client.connect(self.ip, port=self.port, username=arguments['-u'], password=arguments['-p'], pkey=key, timeout=10)
            return True
...
def main():
    global arguments
    arguments = docopt(__doc__, version="V1.0 anOps tool for ssh operation")
    ...
    for hostname, ip_port in get_host_info(arguments['<targets>']):
        ...
        auto_play = AutoTask(hostname, ip, port)
        c = auto_play.create_sshclient()
        if c == 'continue':
            continue
        elif not c:
            break
        # 区别处理 cmd put get参数
        if arguments['cmd']:
            t = threading.Thread(target=auto_play.run_command, args=(arguments['<command>'],))
        elif arguments['put']:
            t = threading.Thread(target=auto_play.sftp_transfer, args=(arguments['<src>'], arguments['<dst>'], 'put'))
        elif arguments['get']:
            t = threading.Thread(target=auto_play.sftp_transfer, args=(arguments['<src>'], arguments['<dst>'], 'get'))
        t.start()
